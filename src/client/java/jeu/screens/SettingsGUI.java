...
public class SettingsGUI extends Screen {
    ...
    ArrayList<FeatureCardWidget> allWidgets;
    ...
    private int getStringScore(String target, String query) {
        ...
        if (target.equals(query)) return 1000;
        if (target.startsWith(query)) return 800 + (query.length() * 10);
        ...
        for (String word : words) {
            if (word.startsWith(query)) return 400 + (query.length() * 5);
        }
